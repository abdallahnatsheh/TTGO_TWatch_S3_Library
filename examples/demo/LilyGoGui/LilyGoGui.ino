...
#define WIFI_PASSWORD         "0522718351Adel0522718351"

#define WIFI_CONNECT_WAIT_MAX (30 * 1000)

#define NTP_SERVER1           "pool.ntp.org"
#define NTP_SERVER2           "time.nist.gov"
#define GMT_OFFSET_SEC        2
#define DAY_LIGHT_OFFSET_SEC  3
...
const char* cn_month[12] = { "Jan","Feb","Mar","Apr","May","Jun","Jul","Aug"," Sep","Oct","Nov","Dec"};
const char* cn_state[5] = {"GM", "GN", "GAN", "GE"};
...
lv_obj_t *setupGUI()
{
    ...
    lv_style_set_text_font(&model_style, &lv_font_montserrat_16);
    ...
    lv_style_set_text_font(&time_style,  &lv_font_montserrat_42);
    ...
    char hour_t[30] = { 0 };
    sprintf(hour_t, "%02d",show_timeinfo.tm_hour+1>=24?show_timeinfo.tm_hour+1-24:show_timeinfo.tm_hour+1);
    lv_label_set_text(hour, hour_t);
    //lv_obj_align_to(hour, view, LV_ALIGN_CENTER, -50, 10);
    lv_obj_set_pos(hour, 8, 100);  
    ...
    lv_style_set_text_font(&dot_style, &lv_font_montserrat_42);
    ...
    lv_style_set_text_font(&second_style, &lv_font_montserrat_20);
    ...
    lv_obj_align_to(second, minute, LV_ALIGN_OUT_RIGHT_BOTTOM, -25, 0);
    ...
    lv_style_set_text_font(&chinese_style, &lv_font_montserrat_24);
    ...
    lv_style_set_text_font(&temp_style, &lv_font_montserrat_12);


    lv_obj_t *temp_text = lv_label_create(view);
    lv_obj_add_style(temp_text, &temp_style, 0);
    // lv_label_set_text(temp_text, "24*C");
    lv_label_set_text_fmt(temp_text,"%.f°C", watch.readAccelTemp());
    ...
    lv_style_set_text_font(&bat_style, &lv_font_montserrat_24);


    lv_obj_t *bat = lv_label_create(view);
    lv_obj_add_style(bat, &bat_style, 0);
    lv_label_set_text_fmt(bat,"%d%%", watch.getBatteryPercent());
    ...
    lv_style_set_text_font(&key_style, &lv_font_montserrat_28);
    ...
    lv_style_set_text_font(&bot_style, &lv_font_montserrat_20);
...
void renew_ui_time(void)
{
    ...
    {
        int hour_temp = show_timeinfo.tm_hour+1>=24?show_timeinfo.tm_hour+1-24:show_timeinfo.tm_hour+1;
        show_timeinfo_old.tm_hour = show_timeinfo.tm_hour;
        sprintf(temp, "%02d",hour_temp);
        lv_label_set_text(hour, temp);
        if(hour_temp>=8 && hour_temp<11)
        {
            lv_label_set_text(state, "GM");
        }
        else if(hour_temp>=11 && hour_temp<=13)
        {
            lv_label_set_text(state, "GN");
        }
        else if(hour_temp>=14 && hour_temp<=17)
        {
            lv_label_set_text(state, "GAN");
        }
        else
        {
            lv_label_set_text(state, "GE");
        }
    }
